...
export function AnalysisCharts() {
  ...
  const getSumCoords = (val: number, idx: number, total: number) => {
    ...
    const y = padding.top + chartH - ((val - minSum) / (maxSum - minSum)) * chartH;
    return { x, y };
  };

  // Generate paths for Sum Line Chart
  const sumPoints = chartData.map((d, i) => getSumCoords(d.analysis!.sum, i, chartData.length));
  const sumPathD = sumPoints.reduce((acc, p, i) => `${acc} ${i === 0 ? "M" : "L"} ${p.x} ${p.y}`, "");
  const sumAreaD = sumPoints.length > 0 
  ...
  const acPoints = chartData.map((d, i) => getAcCoords(d.analysis!.ac, i, chartData.length));
  const acPathD = acPoints.reduce((acc, p, i) => `${acc} ${i === 0 ? "M" : "L"} ${p.x} ${p.y}`, "");
  const acAreaD = acPoints.length > 0
  ...
  return (
    <div style={{ color: "#ffffff", paddingBottom: "20px" }}>
      ...
      >
        <div>
          ...
          <p style={{ fontSize: "0.75rem", color: "var(--text-dim)", marginTop: "4px" }}>
            데이터베이스에 동기화된 역대 당첨 결과를 바탕으로 통계 트렌드를 시각화합니다.
          </p>
        </div>
        ...
        >
          {([30, 50, 100, "all"] as const).map(p => (
            <button
              ...
              style={{
                background: period === p ? "var(--primary-cyan)" : "transparent",
                border: "none",
                borderRadius: "8px",
                padding: "6px 12px",
      ...
      >
        ...
        >
          ...
          <svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", height: "auto" }}>
            ...
            {sumGridLines.map((level, i) => {
              const y = padding.top + chartH - ((level - minSum) / (maxSum - minSum)) * chartH;
              return (
                <g key={i}>
                  <line x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="rgba(255,255,255,0.04)" strokeDasharray="3 3" />
                  <text x={padding.left - 8} y={y + 3} fill="var(--text-dim)" fontSize="9" textAnchor="end">{Math.round(level)}</text>
                </g>
              );
            })}
            <line x1={padding.left} y1={padding.top + chartH} x2={width - padding.right} y2={padding.top + chartH} stroke="rgba(255,255,255,0.08)" />
            ...
            {/* Line Path */}
            <path d={sumPathD} fill="none" stroke="var(--primary-cyan)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ filter: "drop-shadow(0 0 4px rgba(0, 240, 255, 0.4))" }} />
                ...
                <g key={i}>
                  ...
                  {isActive && (
                    <>
                      ...
                      <line x1={p.x} y1={padding.top} x2={p.x} y2={padding.top + chartH} stroke="rgba(0, 240, 255, 0.25)" strokeWidth="1" />
                      {/* Glowing dot */}
                      <circle cx={p.x} cy={p.y} r={6} fill="var(--primary-cyan)" opacity="0.3" />
                      <circle cx={p.x} cy={p.y} r={4.5} fill="#ffffff" stroke="var(--primary-cyan)" strokeWidth="1.5" />
                    </>
                  )}
                  {/* Small static dots for lower density periods */}
                  {chartData.length <= 50 && !isActive && (
                    <circle cx={p.x} cy={p.y} r={2} fill="var(--primary-cyan)" opacity="0.6" />
                  )}
                </g>
              );
            })}
          ...
          {hoveredSumIndex !== null && chartData[hoveredSumIndex] && (
            ...
            >
              <div style={{ color: "var(--text-muted)", fontWeight: "bold" }}>{chartData[hoveredSumIndex].episode}회차</div>
              <div style={{ marginTop: "2px" }}>
                총합: <span style={{ color: "var(--primary-cyan)", fontWeight: "bold" }}>{chartData[hoveredSumIndex].analysis!.sum}</span>
              </div>
            </div>
          )}
        </div>
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "12px" }}>
            ...
            <span style={{ fontSize: "0.75rem", color: "var(--primary-purple)", fontFamily: "monospace" }}>
              범위: {minAc} ~ {maxAc}
            </span>
          </div>
          ...
          <svg viewBox={`0 0 ${width} ${height}`} style={{ width: "100%", height: "auto" }}>
            <defs>
              <linearGradient id="acGrad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="var(--primary-purple)" stopOpacity="0.25" />
            ...
            {acGridLines.map((level, i) => {
              const y = padding.top + chartH - ((level - minAc) / (maxAc - minAc)) * chartH;
              return (
                <g key={i}>
                  <line x1={padding.left} y1={y} x2={width - padding.right} y2={y} stroke="rgba(255,255,255,0.04)" strokeDasharray="3 3" />
                  <text x={padding.left - 8} y={y + 3} fill="var(--text-dim)" fontSize="9" textAnchor="end">{Math.round(level)}</text>
            ...
            {/* Line Path */}
            <path d={acPathD} fill="none" stroke="var(--primary-purple)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ filter: "drop-shadow(0 0 4px rgba(189, 0, 255, 0.4))" }} />
            ...
            {acPoints.map((p, i) => {
              ...
              return (
                <g key={i}>
                  ...
                  {isActive && (
                    <>
                      <line x1={p.x} y1={padding.top} x2={p.x} y2={padding.top + chartH} stroke="rgba(189, 0, 255, 0.25)" strokeWidth="1" />
                      <circle cx={p.x} cy={p.y} r={6} fill="var(--primary-purple)" opacity="0.3" />
                      <circle cx={p.x} cy={p.y} r={4.5} fill="#ffffff" stroke="var(--primary-purple)" strokeWidth="1.5" />
                    </>
                  )}
                  {chartData.length <= 50 && !isActive && (
                    <circle cx={p.x} cy={p.y} r={2} fill="var(--primary-purple)" opacity="0.6" />
                  )}
                </g>
              );
            })}
          ...
          {hoveredAcIndex !== null && chartData[hoveredAcIndex] && (
            ...
            >
              <div style={{ color: "var(--text-muted)", fontWeight: "bold" }}>{chartData[hoveredAcIndex].episode}회차</div>
              <div style={{ marginTop: "2px" }}>
                AC값: <span style={{ color: "var(--primary-purple)", fontWeight: "bold" }}>{chartData[hoveredAcIndex].analysis!.ac}</span>
              </div>
            </div>
          )}
        </div>
        ...
        >
          <span style={{ fontSize: "0.85rem", fontWeight: "bold", color: "var(--text-main)", display: "block", marginBottom: "16px" }}>
                ...
                <div key={i} style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.78rem" }}>
                    ...
                    <span style={{ color: "var(--text-dim)" }}>
                      평균 <span style={{ color: "var(--text-main)", fontWeight: "bold" }}>{d.avg}개</span> ({percent}%)
                    </span>
                  </div>
                  <div
                    style={{
          ...
          >
            {[
              ...
              return (
                ...
                >
                  <span style={{ fontSize: "0.8rem", fontWeight: "bold", color: t.color }}>{t.avg}개</span>
                  <div
                    style={{
      ...
      >
        <span style={{ fontSize: "0.88rem", fontWeight: "bold", color: "var(--text-main)", display: "block", marginBottom: "12px" }}>
        ...
        <div style={{ display: "flex", gap: "24px", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}>
          ...
          <div style={{ position: "relative", width: "80px", height: "80px", flexShrink: 0 }}>
            ...
            >
              <span style={{ fontSize: "0.95rem", fontWeight: "bold", color: "#ffffff", fontFamily: "monospace" }}>
                {statsSummary?.oddPct}:{statsSummary?.evenPct}
              </span>
              <span style={{ fontSize: "0.55rem", color: "var(--text-dim)" }}>홀:짝</span>
            </div>
          </div>
            ...
            >
              <div
                style={{
                  ...
                  background: "linear-gradient(90deg, #bd00ff 0%, #e879f9 100%)",
                  transition: "width 0.6s cubic-bezier(0.1, 0.8, 0.3, 1)",
                }}
              />
              <div
                style={{
                  ...
                  background: "linear-gradient(90deg, #22d3ee 0%, #00f0ff 100%)",
                  transition: "width 0.6s cubic-bezier(0.1, 0.8, 0.3, 1)",
                }}
              />
            </div>
        ...
        <p style={{ fontSize: "0.75rem", color: "var(--text-dim)", marginTop: "12px", lineHeight: "1.4", margin: 0, textAlign: "left" }}>
          선택한 최근 <b>{period === "all" ? "전체" : `${period}회차`}</b> 동안 출현한 전체 당첨번호 공들 중 홀수와 짝수의 비율입니다. 이론상 로또 홀짝 비율의 확률적 기댓값은 약 50:50에 수렴합니다.
        </p>
      </div>
    </div>
  );
